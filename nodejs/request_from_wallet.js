...
const execute = async () => {
    ...
    const gasAddress = "0x09ae1a5ddfd481cfd3cc4390b0e08a0832709a06";
    ...
    //const country = "🇩🇪";
    //const toAddress = "0xfaf918d035e6941a8d19a5e087ad6d3c1c2653c1";

    //const name = "Алекс из Новосибирска 🥶";
    //const country = "🇷🇺";
    //const toAddress = "0x771ddb07222a1f9442c91cf04f64f3164771bb62";

    //const name = "台湾 is not 中国";
    //const country = "🇹🇼";
    //const toAddress = "0x7757a1f517d4680dba5d0ae9c984d3d394cc4a30";
    ...
    const country = "🇿🇦";
    const toAddress = "0x09ae1a5ddfd481cfd3cc4390b0e08a0832709a06";

    // how many confirmations do we want to wait for
    const waitForConfirmations = 3;

    // how much do we want to spend
    const spendeFromFaucetInETH = "1"; // NRG
    ...
    const signer = provider.getSigner(gasAddress);
    ...
    if (name) {
        faucet.getRecipientName(toAddress).catch((e) => { console.log("getRecipientName failed:", e); }).then((n) => { currentName = n; });
    }

    console.log(toAddress + ": '" + name + "' is requesting", spendeFromFaucetInETH, "NRG");
    requestDonation(faucet, toAddress, name, ethers.utils.toUtf8Bytes(country), ethers.utils.parseEther(spendeFromFaucetInETH).toString(), waitForConfirmations)
        .catch((e) => {
            ...
            if (r) {
                if (r.status) {
                    ...
                    if (currentName && name && currentName !== name) {
                        // update donor name
                        faucet.setRecipientName(name).catch((e) => { console.log("SetName failed:", e); }).then((n) => { console.log("Updated your name from '", currentName, "' to '", name + "'"); });
                    }
                    console.log("****************************************");
                    console.log("Request from Faucet succesfull 🤑");
                    ...
                    console.log("Thank you'", name, "'for requesting", parseFloat(spendeFromFaucetInETH) + "NRG");

                } else {
                    console.log("requestDonation returned status false");
                    console.log(r);
                }
            }
        });
...
// recipientCountry must be an hex array, eg from ethers.utils.toUtf8Bytes("🇦🇹")
const requestDonation = async (contract, toAddress, recipientName, recipientCountry, wei, waitForConfirmations) => {
    let result;
    let lastConfirms = -2;
    try {
        ...
        const txResponse = await contract.requestDonation(toAddress, wei, recipientName, recipientCountry, { gasLimit: 300000, });
